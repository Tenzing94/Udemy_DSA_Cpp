...
int main(){
    ...
    Display(*arr3); // Since 'arr3' is a pointer, we need to dereference it
    struct Array *arr4 = Intersection(&arr1, &arr2);
    Display(*arr4); // Since 'arr4' is a pointer, we need to dereference it
    struct Array *arr5 = Difference(&arr1, &arr2);
    Display(*arr5); // Since 'arr5' is a pointer, we need to dereference it
    return 0;
}
