...
struct Node {
    int data;
    struct Node *next;
}*first = NULL, *second = NULL, *third = NULL; // this is a global pointer
...
// function for creating 'second' LL
...
void merge(struct Node *p, struct Node *q) {
    ...
    while (p && q) {// while 'p' and 'q' are not NULL
        if (p->data < q->data) {
            last->next = p;
            last = p;
            p = p->next;
...
int isLoop(struct Node *p) {
    ...
    if (p == q) // if so, then there is a loop in LL
        return 1;
    else
        return 0;
...
int main() {
    struct Node *t1, *t2; // we will use these to create loop in LL
    int A[] = {10,20,30,40,50};
    create(A,5); // creates the first LL in heap
    
    t1 = first->next->next; // t1 will point to the Node with '30'
    t2 = first->next->next->next->next; // t1 will point to the Node with '50'
    t2->next = t1; // Now, there is a loop in the LL
